...
uint32_t lcd_read_point(uint16_t x, uint16_t y)
{
    ...
    g <<= 8;
    return (((r >> 11) << 11) | ((g >> 10) << 5) | (b >> 11));  /* ILI9341/NT35310/NT35510/ST7789��Ҫ��ʽת��һ�� */
}
...
void lcd_init(void)
{
    ...
    /* FSMC��ʱ����ƼĴ��� */
    fsmc_read_handle.AddressSetupTime = 0x0F;           /* ��ַ����ʱ��(ADDSET)Ϊ15��fmc_ker_ck 1/168=6*16=96ns */
    fsmc_read_handle.AddressHoldTime = 0x00;
    fsmc_read_handle.DataSetupTime = 60;                /* ���ݱ���ʱ��(DATAST)Ϊ60��fmc_ker_ck=6*60=360ns */
                                                        /* ��ΪҺ������IC�Ķ����ݵ�ʱ��,�ٶȲ���̫��,�����Ǹ�������оƬ */
    fsmc_read_handle.AccessMode = FSMC_ACCESS_MODE_A;   /* ģʽA */
    /* FSMCдʱ����ƼĴ��� */
    fsmc_write_handle.BusTurnAroundDuration = 0;
    fsmc_write_handle.AddressSetupTime = 9;             /* ��ַ����ʱ��(ADDSET)Ϊ9��fmc_ker_ck=6*9=54ns */
    fsmc_write_handle.AddressHoldTime = 0x00;
    fsmc_write_handle.DataSetupTime = 9;                /* ���ݱ���ʱ��(DATAST)Ϊ9��fmc_ker_ck=6*8=54ns */
                                                        /* 9��fmc_ker_ck��fmc_ker_ck=168Mhz��,ĳЩҺ������IC��д�ź���������Ҳ��50ns */
    ...
    /* ��ʼ������Ժ�,���� */
    if (lcddev.id == 0x9341 || lcddev.id == 0x1963 || lcddev.id == 0x7789)  /* ������⼸��IC, ������WRʱ��Ϊ��� */
    {
        /* ��������дʱ����ƼĴ�����ʱ�� */
...
void lcd_show_float(uint16_t x, uint16_t y, float num, uint8_t size, uint16_t color)
{
    char temp[64];
    sprintf(temp, "%f   ", num);
    lcd_show_string(x, y, 200, 200, size, temp, color);//VPP���ֵ
}
